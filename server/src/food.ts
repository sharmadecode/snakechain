import { NUM_COLORS } from "./config.js";
...
export class FoodManager {
  ...
  /** events flushed by the session every tick */
  spawned: number[] = [];
  removed: number[] = [];
  /** item refs parallel to spawned/removed (for grid sync + interest filtering) */
  ...
  flushEvents(): { s: number[]; r: number[]; sItems: FoodItem[]; rItems: FoodItem[] } {
